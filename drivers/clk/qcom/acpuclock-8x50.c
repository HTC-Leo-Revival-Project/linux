...
struct cpufreq_frequency_table_acpuclock {
...
static DEFINE_PER_CPU(struct cpufreq_frequency_table_acpuclock *, cpufreq_show_table);

void cpufreq_frequency_table_acpuclock_get_attr(struct cpufreq_frequency_table_acpuclock *table,
				      unsigned int cpu)
{
	pr_debug("setting show_table for cpu %u to %p\n", cpu, table);
	per_cpu(cpufreq_show_table, cpu) = table;
}
EXPORT_SYMBOL_GPL(cpufreq_frequency_table_acpuclock_get_attr);

static struct cpufreq_frequency_table_acpuclock freq_table[24];
				 ...
				 enum setrate_reason reason)
{
	...
	if (strt_s->axiclk_khz != tgt_s->axiclk_khz) {
		...
		if (res < 0)
			pr_warn("Setting AXI min rate failed (%d)\n", res);
	}
	...
	if (tgt_s->vdd < strt_s->vdd) {
		...
		if (res)
			pr_warn("Unable to drop ACPU vdd (%d)\n", res);
	}
out:
	if (reason == SETRATE_CPUFREQ)
...
static void  acpuclk_hw_init(void)
{
	...
	if (res < 0)
		pr_warn("Enabling AXI clock failed (%d)\n", res);
}
...
static void  acpu_freq_tbl_fixup(void)
{
	...
	case 0x10:
		...
		break;
	default:
		pr_warn("Invalid efuse data (%x) on Max ACPU freq!\n",
				tcsr_spare2);
		goto skip_efuse_fixup;
	}
...
static int  acpuclk_8x50_probe(struct platform_device *pdev)
{
	...
	cpufreq_table_init();
	cpufreq_frequency_table_acpuclock_get_attr(freq_table, smp_processor_id());
	return 0;
}
...
int __init acpuclk_8x50_init(void)
{
	return platform_driver_register(&acpuclk_8x50_driver);
}
postcore_initcall(acpuclk_8x50_init);
//TIMER_OF_DECLARE(qsd8k_acpuclk, "qcom,qsd8k-acpuclk", acpuclk_8x50_init);
